    ...
    public class Desktop
    {
        ...
        public void UpdateLayout()
        {
            ...
            for (var i = ChildrenCopy.Count - 1; i >= 0; --i)
            {
                ...
                w.ProcessWidgets(UpdateLayoutProcessWidgets, active);
        ...
        private bool UpdateLayoutProcessWidgets(Widget widget, bool active)
        {
        ...
        internal void ProcessWidgets(Func<Widget, object, bool> operation)
        {
            for (var i = ChildrenCopy.Count - 1; i >= 0; --i)
            {
                var w = ChildrenCopy[i];
                var result = w.ProcessWidgets(operation, (object)null);
                if (!result)
                {
                    return;
                }
